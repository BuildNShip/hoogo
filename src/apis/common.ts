...
export const getBingoMatrix = async (
    ...
    try {
        const response = await publicGateway.post(
            commonUrls.getBingoMatrix(eventName as string) +
...
export const postUserInput = async (
    ...
    publicGateway
        .post(
            commonUrls.userInput(eventName),
            {
                ticket_code: ticketCode,
                letter,
                image: compImage,
        ...
        .catch((error) => {
            toast.error(
                error?.response?.message.general[0] || "User was not added"
            );
        })
        .finally(() => {
...
export const validateTicketCode = async (
    ...
    return publicGateway
        .post(commonUrls.validateTicket(eventName), {
